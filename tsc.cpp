...
#define JUMP_IF(cond) \
	{	\
        ...
        for(;;) {
                ...
                case OP_FAI:
                        fade.Start(FADE_IN, parm[0], SPR_FADE_DIAMOND);
                        return;
                case OP_FAO:
                        fade.Start(FADE_OUT, parm[0], SPR_FADE_DIAMOND);
                        return;
                case OP_FLA:
                        flashscreen.Start();
                        break;
                ...
                case OP_EVE:
                        JumpScript(parm[0]);
                        break;		// unconditional jump to event
                ...
                case OP_SKPLUS:
                        game.skipflags[parm[0]] = 1;
                        break;
                case OP_SKMINUS:
                        game.skipflags[parm[0]] = 0;
                        break;
                case OP_SKJ:
                        JUMP_IF(game.skipflags[parm[0]]);
                        break;

                case OP_PSPLUS:
                        textbox.StageSelect.SetSlot(parm[0], parm[1]);
                        break;
                ...
                case OP_ANP:
                        NPCDo(parm[0], parm[1], parm[2], DoANP);
                        break;
                case OP_CNP:
                        NPCDo(parm[0], parm[1], parm[2], DoCNP);
                        break;
                case OP_DNP:
                        NPCDo(parm[0], parm[1], parm[2], DoDNP);
                        break;
                ...
                case OP_WAS:
                        ...
                        return;	// wait until player has blockd

                case OP_SMP:
                        map.tiles[parm[0]][parm[1]]--;
                        break;
                ...
                case OP_AEPLUS:
                        RefillAllAmmo();
                        break;		// refills missiles

                case OP_INI:
                        game.switchstage.mapno = NEW_GAME;
                        break;		// restart game from beginning
                case OP_STC:
                        niku_save(game.counter);
                        break;
...
const char *DescribeCSDir(int csdir)
{
        ...
        case 5:
                return "NO_CHANGE";
        default:
                return stprintf("Invalid CS Dir %d", csdir);
        }
}
